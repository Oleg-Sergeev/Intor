...
public class GUITools : Editor
{
    ...
    public static void AnchorsToCorners()
    {
        foreach (Transform transform in Selection.transforms)
        {
            ...
            var newAnchorsMin = new Vector2(rect.anchorMin.x + rect.offsetMin.x / parentRect.rect.width,
                                                rect.anchorMin.y + rect.offsetMin.y / parentRect.rect.height);

            var newAnchorsMax = new Vector2(rect.anchorMax.x + rect.offsetMax.x / parentRect.rect.width,
    ...
    [MenuItem("UGUI/Show Offset and Distance of objects %G")]
    ...
    private static void MirrorHorizontally(bool mirrorAnchors)
    {
        foreach (Transform transform in Selection.transforms)
        {
            ...
            if (mirrorAnchors)
            {
                var oldAnchorMin = rect.anchorMin;
